 ...
 * 添加商品
 */
// categoryId, name, subtitle, subImages, detail, price, stock, status
exports.insertProductData = value => {
	let _sql = 'insert product set categoryId=?,name=?,subtitle=?,subImages=?,detail=?,price=?,stock=?,status=?;';
	return query(_sql, value);
};
 ...
 * 更新商品
 */
exports.updateProductById = value => {
	let _sql =
		'update product set categoryId=?,name=?,subtitle=?,subImages=?,detail=?,price=?,stock=?,status=? where id=?;';
	return query(_sql, value);
};
 ...
 * 商品列表
 */
exports.findAllProduct = page => {
	let start = (page - 1) * 10;
	let _sql = 'SELECT * FROM product limit ' + start + ',10';
	return query(_sql);
};
/**
 *  商品详情
